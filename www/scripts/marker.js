...
function disableViewerCanvas (viewer) {
	...
	if (viewer.viewCubeUi.homeViewContainer) {
		if (viewer.viewCubeUi.infoButton) viewer.viewCubeUi.infoButton.style.display = 'none';
...
function placeMarkerOnCanvas (evt, skipViewerUpdate) {
	...
	if (!(skipViewerUpdate))		{ updateCameraToMarker(); }
...
function trackMarker (evt) {
	if (marker.isTracking)		{ updateCameraToMarker(false, false); }	else if (marker.isRotating)		{ updateCameraToMarker(true, false); }
}
...
function updateMarkerToCamera () {
	if (marker.isTracking() || marker.isRotating())		{ return; }
	...
	var onmarkerdrag = function (evt) {
		...
		if (currentTimestamp - lastTimestamp > REFRESHINTERVAL) {
			if (isTracking) {
				...
				d = directionMap.calcMouseDirection(evt.clientX, evt.clientY);
				directionMap.currentDirection = d;

				angleToxAxis = directionMap.angleBetween(d, [1, 0]);
				rotateMarker((Math.PI / 2 - angleToxAxis), 'rad');
			}
...
Marker.prototype.setPosition = function (offsetLeft, offsetTop) {
	if (this.marker.style.display === 'none')		{ this.showMarker(); }
...
Marker.prototype.addEventListenerOnMarker = function (eventName, callFunc) {
	...
	if (eventName in externalEvtHandler)		{ externalEvtHandler[eventName].push(callFunc); }
};
...
DirectionMap.prototype.angleBetween = function (d1, d2) {
	...
	if (d1[1] < 0)		{ angle1 = 2 * Math.PI - angle1; }
	if (d2[1] < 0)		{ angle2 = 2 * Math.PI - angle2; }
