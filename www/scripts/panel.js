...
function pushStateToTableList (blobURL, modelIndex, stateObj) {
	...
	img.onclick = function (evt) {
		if (panelDisabled) return; // disabled when the viewer is still loading
...
function loadViewStateList () {
	for (var i = 0; i < viewModels.length; i++) {
		var objStr = window.localStorage.getItem(viewModels[i].id);
		if (objStr && objStr.length > 0) {
			...
			for (var j = 0; j < viewStates.length; j++) {
				...
				var dataURI = window.localStorage.getItem(viewStates[j].id);
...
function updateLocalStorage () {
	for (var i = 0; i < viewModels.length; i++) {
		...
		window.localStorage.setItem(model, objStr);
	};
}
...
function saveBlobToLocalStorage (blob, name) {
	...
	reader.onload = function (evt) {
		...
		window.localStorage.setItem(name, fileUploadData);
	};
